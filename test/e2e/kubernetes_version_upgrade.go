...
func KubernetesVersionUpgradeSpec(ctx context.Context, inputGetter func() CommonSpecInput) {
	...
	BeforeEach(func() {
		...
		Expect(input.E2EConfig.Variables).To(HaveKey(UpgradedKubernetesVersion))
		Expect(input.E2EConfig.Variables).To(HaveValidVersion(input.E2EConfig.GetVariable(UpgradedKubernetesVersion)))
	...
	It("Should successfully upgrade kubernetes versions when there is a change in relevant fields", func() {
		...
		clusterctl.ApplyClusterTemplateAndWait(ctx, clusterctl.ApplyClusterTemplateAndWaitInput{
			...
			ConfigCluster: clusterctl.ConfigClusterInput{
				...
				Namespace:                namespace.Name,
				ClusterName:              clusterName,
				KubernetesVersion:        input.E2EConfig.GetVariable(UpgradedKubernetesVersion),
				ControlPlaneMachineCount: pointer.Int64Ptr(1),
				WorkerMachineCount:       pointer.Int64Ptr(1),
			},
			WaitForClusterIntervals:      input.E2EConfig.GetIntervals(specName, "wait-cluster"),
